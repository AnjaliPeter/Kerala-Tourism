...
// email,phone.no,password format validation
function validate(){
    ...
    if (phoneno.test(num)||phone.test(num)){
        // alert("Valid Phone No. format");
    }
    else{
        alert("Invalid Phone No.! (should contain 10 digits,eg:-xxxxxxxxxx,xxx-xxx-xxxx,xxx.xxx.xxxx,xxx xxx xxxx)");
        return false;
    }

    // password format validation
    if (str.match(/[a-z]/g) && str.match(/[A-Z]/g) && str.match(/[0-9]/g) && str.match(/[^a-zA-Z\d]/g) && str.length >= 8){
        alert("Your acoount is ready. You can LogIn now!");
        return true;
    }
    else{
